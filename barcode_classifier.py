...
class BarcodeClassifier:
    ...
    def _aruco_detect_markers(self):
        ...
        self.bbox, self.ids, self.rejected = aruco.detectMarkers(self._gray, self._aruco_dict,
    ...
    def _cv_show_detected_redlines(self):
        ...
        cv2.rectangle(self.observation, self._red_window_bound[0], self._red_window_bound[1], color=(255, 0, 0),
                      thickness=1)
    ...
    def get_absolute_paths(self, heading):
        if self.ids is None:
            return []
    ...
    def get_oriented_paths(self):
        if self.ids is None:
            return []
        return [path for path in BarcodeClassifier.PATHS[max(self.ids[0])] if path != Direction.DOWN]
